...
UPDATE (TON payment mission: Activate Web3)
- Mission `Activate Web3` must be a payment mission (treat it as a product/item purchase), not a manual mission.
- Required flow:
  1) Mission starts TON payment from TMA for 1 TON (or configured amount).
  2) App obtains transaction id.
  3) Backend verifies transaction and only then rewards user.
- Verification rules (server-side):
  - tx sender must be user's connected wallet address.
  - tx recipient must be `TON_OUTPUT_ADDRESS`.
  - tx amount must be >= mission amount (default 1 TON).
  - confirmations must be > `TON_MIN_CONFIRMATIONS` (default 1).
- If verification fails, persist purchase as `declined/failed`, reset mission card state to initial state, allow user to start mission again.
- If tx id exists but tx is not finalized yet, mission card must show `Claim` action that calls the same verification/reward endpoint.
  - On success => mission completed + reward granted.
  - On declined => reset mission state so user can restart.
- In async states (pending/confirmed/declined), send Telegram notification to the user.
- Payment APIs must be split out into dedicated endpoints/services (not mixed into generic mission completion logic).


UPDATE (TON lite node deployment defaults)
- Remove all references to public shared-rate providers; do not use them as default provider.
- Default TON API host must be `https://ton.kingofthehill.pro` for backend transaction checks.
- Keep `TON_API_BASE_URL` configurable, but prefill it with `https://ton.kingofthehill.pro` in `.env.example`.
- Repository must include `docker-compose.yml` to run a self-hosted TON lite stack on server (TON HTTP API + required services).
